...
- 元の16:9映像を縦長キャンバスの中央（1080x608、上下に656pxずつの帯）に配置し、
  ...
  使うとインパクトが不足しがちだったため**、ショート特有の「一瞬でスクロール
  を止めてもらう」ための、より強い言い回しを考える専用エージェントに分けた。
  文字サイズも帯の高さに対して十分な大きさ（既定で帯の高さの2割程度を
  占める）になるよう大きくしている）
- 実際に生成した動画で、指定秒数付近の無音区間に正しく切り出し位置が
  調整されること、末尾で音声が滑らかにフェードアウトすること、文字が
  十分な大きさで描画されることを検証済み
