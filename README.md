...
flowchart TD
    ...
    ShortsData["shorts_data.json<br/>(シーン見出し・箇条書き+話者発話区間)"] -.中段/下段の再構成.-> CS
...
flowchart TD
    ...
    Integration -->|スコア未達 or issuesあり: 該当箇所を修正| Voicevox
    Integration -->|スコア達成 かつ issuesなし| Title["タイトルエージェント<br/>(--title未指定時のみ、単発生成)"]
...
「スコアは閾値超えだがissuesに軽微な指摘が残っている」ケース（例:
まとめスライドとエンディングスライドのscene_numberが両方19番で重複、
...
読めてしまう問題が実際に発生した（例:台本は「人間の検知率13.6%→5%
に対しAIは89%」という対比なのに、主語を省いて「検知率わずか5%」と
すると、AI側の数値が悪化しているかのように読め、台本の主張と正反対の
印象を与えてしまっていた）。`title_agent.py`/`thumbnail_agent.py`の
プロンプトに、数値を使う場合は主語を省略しない旨の制約を追加して対応した
（プロンプトの調整であり生成結果を保証するものではないため、再生成した
タイトル・サムネイルで都度確認することを推奨する）。
...
closed(口を閉じた)/open(口を開けた)の立ち絵は別々に生成された素材で
シルエットが厳密には一致しないため(手の位置や髪の一房が数px違う、等)、
表示区間を単純に「常時表示+上から重ねる」実装のままだと、上のレイヤーが
下のレイヤーのシルエットを完全に覆いきれない部分(はみ出た手や髪)が
同時に描画され続けて二重に見えてしまう不具合があった（姉妹プロジェクト
code-video-toolkit-appの実機で確認・修正した不具合と同じ原因）。
通常表情のclosed/openと感情差分のclosed/openそれぞれについて、表示区間が
互いに重ならない(排他的な)ffmpegの`enable`式になるよう修正済み
（`video_assembler.subtract_ranges`がこの排他区間の計算を担う）。
...
└── agents/
    ...
    └── shorts_agent.py         # ショート上段に固定表示するフック文言(hook_text)の生成(単発呼び出し)
```
...
- `SCORE_THRESHOLD`: この点数(0-100)以上で合格とみなす。総合エージェントの整合性スコアにも適用される（デフォルト80。
  ただし総合エージェントはスコアがこの値以上でも`issues`が残っている間は修正ループを継続する）
...
- 画像生成は2段階のフォールバック構成:
  1. **推奨**: `GEMINI_API_KEY`が使えれば、Geminiに背景・イラスト・文字を
     ...
     プロンプトの調整は2段階の試行錯誤を経ている。**最初はキャッチコピーの
     文字列だけを渡していたため、単に「文字+汎用的な背景」にしかならず
     見栄えが良くなかった**。そこで`visual_summary`（動画の核心的な内容を
     2〜3文で要約したもの）を渡し、比較パネルやアイコンを積極的に
     デザインさせる指示に変えたところ、**今度は情報量が多すぎて逆にクリック
     されにくいサムネイルになった**（実際に生成した画像で、小さすぎて読めない
     文字が並ぶ複数パネル構成になることが確認された。YouTubeのサムネイルは
     120〜350px程度の小さいサイズで一瞬見て判断されるものなので、詳細な
     図解は伝わらない）。現在は`visual_summary`を「複数パネルを作る材料」
     ではなく「1つのシンプルな視覚要素を選ぶための参考情報」として使い、
     「1つの焦点+大きな文字」に情報量を絞るよう明示している。さらに、
     実際に良質なサムネイルを集めたギャラリーサイトの画像を確認したところ、
     共通していたのは(1)濃い1色の力強い背景（オレンジ・黄色・黒など。
     dark navyに限らない）(2)視覚要素は写真1枚・イラスト1体・図形1つの
     いずれかに絞られている(3)補助要素があっても小さいバッジ程度、という
     3点だったため、これもプロンプトに反映した
...
という作業を手動で行っていたが、これを自動化した。さらにその後、元の16:9映像を
縮小して中央に貼るだけの構成だと、スマホ画面上でスライドの文字がほとんど
読めなくなる問題があったため、映像を縮小するのではなく、縦長キャンバス向けに
直接テキスト・キャラクターを大きく再構成する現在の3段構成に変更した。
...
- **上段**（固定150px）: `shorts_agent`が生成する`hook_text`を動画全体を通して
  常時表示する。サムネイル用のキャッチコピーをそのまま使うとインパクトが
  不足しがちだったため（**実際にそう指摘された**）、ショート特有の「一瞬で
  スクロールを止めてもらう」ための、より強い言い回しを考える専用エージェントに
  分けている
- **中段**: 再生中のシーンに対応するスライドの見出し・箇条書きを、シーンが
  切り替わるたびに拡大表示する。スライドはPNGとして焼き込んだ時点で文字情報が
  失われるため、`render-video`が書き出す`shorts_data.json`（`slides/manifest.json`
  に残しておいたスライドの元データから再構成）を読んで文字を直接描画し直している
- **下段**: つむぎ・ずんだもんの立ち絵を、本編と同じ口開閉オーバーレイ（発話区間に
  合わせて口が動く）で並べて配置する。発話区間のタイミングも`shorts_data.json`
  から読む

このため、**`shorts_data.json`が無い動画（旧バージョンの`render-video`で生成した
過去の動画）はショート化できない**。`create-shorts`はその場合、分かりやすい
エラーメッセージを出して終了するので、`render-video`を最新版で再実行してから
`create-shorts`を実行し直す。

切り出す区間(何秒目までを使うか)の決め方はレイアウト変更前と同じ:
...
- デフォルトでは`--scene 1`（シーン1=概要パートの終わりまで）を切り出す。
  ...
  （`scene_boundaries.json`が無い場合、`--duration`の目安秒数+無音検出に
  フォールバックする。末尾には短い音声フェードアウトを必ずかける）
- 切り出した映像は視聴維持率を意識して1.5倍速にする（本編には適用しない）。
  スライド文字・キャラクターの口パクのタイミングも、この速度変換後の
  タイムラインに合わせて再計算している
- キャラクターの立ち絵素材は、頭上に透明な余白があるPNGだとショートの拡大
  表示では不自然な空白として目立つ（本編の口パクオーバーレイは表示が小さく
  気にならなかった）ため、bounding boxでトリミングしてから使っている
- 実際に生成した動画で、上段・中段・下段が正しい位置に合成されること、
  シーンが切り替わるたびに中段のテキストが変わること、下段のキャラクターの
...
### `shorts_data.json が見つかりません`(create-shorts実行時)

`create-shorts`が要求する`shorts_data.json`（シーンごとの見出し・箇条書きと
話者ごとの発話区間を記録したファイル。上段=フック文言/中段=スライド文字/
下段=キャラクターの3段レイアウトの再構成に使う）は、それが導入された時点より
前の`render-video`実行結果には存在しない。`render-video`を最新版で再実行して
`final_video.mp4`ごと作り直してから`create-shorts`を実行する
（過去に生成した動画への遡及対応はしていない）。
