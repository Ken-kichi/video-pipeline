...
- セリフの文言自体は台本から変更しない（読み上げ用の抽出のみ行う）。
  一人称（つむぎ「あーし」/ずんだもん「ボク」）・語尾・言い回し・表記を
  含め、一字一句そのままコピーする。要約・言い換え・表記の統一・
  誤字の修正・カタカナ変換などは一切行わない
...
- セリフ本文が台本の文言と一言一句完全に一致しているか（一人称・語尾・
  言い回し・表記の書き換えがないか）。特につむぎの一人称「あーし」と
  ずんだもんの一人称「ボク」が入れ替わっていないかは重点的に確認する
