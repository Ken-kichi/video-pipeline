...
- 【重要・必須】台本冒頭0:00〜1:00の概要パートは、この動画から切り出してYouTube
  ...
  スライドを2シーン以上にわたって使い回さないこと（概要パートのシーンが8〜10個ある
  想定なので、概要パートだけで最低8〜10枚のスライドになる計算になる）
- スライド枚数は動画全体で目安20〜26枚程度にする（概要パートの8〜10枚に加えて、
  残り約9分の詳細解説パートにも1シーンあたり最低1枚を割り当てるとこの枚数感になる。
  概要パートのシーン分割をさらに密にした分、全体の枚数目安も従来（16〜22枚）より
  増やしている）
...
- スライド枚数が多すぎ/少なすぎないか（目安20〜26枚程度。概要パートの8〜10枚を
  含めた全体の枚数感なので、以前の16〜22枚という基準では判断しないこと）
- 【重要】概要パート(0:00〜1:00)に含まれる各シーンに、それぞれ専用のスライドが
  ...
  ことは離脱の主要因になる）
- 【重要】詳細解説パートで、1つのスライドが長時間（目安60秒以上）表示され
  続けている箇所がないか（台本のシーン滞在時間が長い場合、そのシーン内でも
  スライドを追加して分割すべき。あれば減点し、どのシーンに追加すべきか指摘する）
