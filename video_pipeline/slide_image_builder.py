...
def build_title_slide(
    ...
    total_height = (
        _block_height(title_lines, title_font) + 40 +
...
def _render_bullets(img: Image.Image, draw: ImageDraw.ImageDraw, slide_data: dict) -> None:
    ...
    title_lines = _wrap_text(draw, slide_data.get(
        "title", ""), title_font, body_max_width)
    y = 90
    y = _draw_lines(draw, title_lines, title_font, MARGIN, y, TITLE_COLOR)
...
def _render_stat(img: Image.Image, draw: ImageDraw.ImageDraw, slide_data: dict) -> None:
    ...
    if title:
        ...
        _draw_lines_centered(draw, title_lines, title_font,
                             SLIDE_WIDTH // 2, 110, SUBTITLE_COLOR)
    ...
    if stat_label:
        ...
        _draw_lines_centered(draw, label_lines, label_font,
                             SLIDE_WIDTH // 2, band_bottom + 50, BODY_COLOR)
...
def _render_quote(img: Image.Image, draw: ImageDraw.ImageDraw, slide_data: dict) -> None:
    ...
    context_lines = _wrap_text(
        draw, quote_context, context_font, SLIDE_WIDTH - MARGIN * 2) if quote_context else []
    ...
    if context_lines:
        y += 40
        _draw_lines_centered(draw, context_lines, context_font,
                             SLIDE_WIDTH // 2, y, SUBTITLE_COLOR)
...
def _render_comparison(img: Image.Image, draw: ImageDraw.ImageDraw, slide_data: dict) -> None:
    ...
    title_lines = _wrap_text(draw, slide_data.get(
        "title", ""), title_font, SLIDE_WIDTH - MARGIN * 2)
    y_title = 90
    ...
    draw.line([(divider_x, content_top), (divider_x, SLIDE_HEIGHT - 120)],
              fill=ACCENT_COLOR_SOFT, width=3)
    ...
    for x, label_key, bullets_key in (
        ...
        y = content_top
        label_lines = _wrap_text(draw, slide_data.get(
            label_key, ""), label_font, column_width)
