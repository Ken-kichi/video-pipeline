...
READING_OVERRIDES: dict[str, str] = {
    # 「架空」を含む単語(架空の等)が、後述の「空の」ルールに先に食われて
    # 壊れないよう、より具体的な(長い)パターンを先に置く。辞書の反復順序が
    # そのまま適用順序になるため、この並び順に意味がある。
    "架空": "かくう",
    "空の": "からの",
    "誤分類": "ごぶんるい",
    # 「値」は単独では「あたい」だが、「数値」「戻り値」「価値」「値段」等の
    # 複合語では読みが変わる(例: 戻り値=もどりち)。単純な文字列置換で
    # 「値」全体を置き換えると複合語を壊すため、単独用法として安全に判断できる
    # 「この値」「その値」のパターンだけに限定して追加している。他の言い回し
    # (例: 「予測値が」)で読み間違いが起きた場合は、その完全なフレーズを
    # ここに追加すること
    "この値": "このあたい",
    "その値": "そのあたい",
}
